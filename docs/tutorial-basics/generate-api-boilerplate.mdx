...
Wouldn't it be cool if you could also generate all the boilerplate code for your API? You are about to do just that using Go. Of course, you will need to have [Go installed](https://go.dev/dl/).
...
generates:
  ...
  pkg/urlshortener/fiber.go:
    module: '@apexlang/codegen/go'
    visitorClass: FiberVisitor
  pkg/urlshortener/grpc.go:
    module: '@apexlang/codegen/go'
    visitorClass: GRPCVisitor
    config:
...
func NewShortenerImpl(log logr.Logger, repository Repository) *ShortenerImpl {
