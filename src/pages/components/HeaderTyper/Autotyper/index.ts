...
function defaultTextListenerType(currentText: string): void {}
function defaultIsTypingListenerType(isTyping: boolean): void {}
...
export class AutoTyper {
  ...
  constructor(config: CreateAutoTyperConfigInterface = {}) {
    ...
    this.textListener = config.textListener || defaultTextListenerType;
    this.isTypingListener = config.isTypingListener || defaultIsTypingListenerType;
    this.queue = [];
    this.executedQueue = [];
    this.isTyping = false;
    this.isActive = false;
